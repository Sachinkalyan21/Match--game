...
class App extends Component {
  ...
  statusChange = () => {
    ...
    if (time === !0) {
      this.setInterval(prevState => ({time: prevState.time - 1}))
    } else {
  ...
  imageClick = thumbnailUrl => {
    ...
    if (imageUrl === imgUrl) {
      const newImgUrl =
        imagesList[Math.floor(Math.random() * imagesList.lengh)].imageUrl
              ...
              <ul className="thumbnai-images">
                {thumbnailList.map(eachObject => (
                  <li key={eachObject.id}>
                    ...
                    >
                      <img
                        src={eachObject.thumbnail.thumbnailUrl}
